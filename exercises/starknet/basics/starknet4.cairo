...
mod LizInventory {
    ...
    fn add_stock(product: felt252, new_stock: u32) {
        ...
        assert(caller == contract_owner::read(), 'caller is not contract owner');
        let old_inventory = inventory::read(product);
        let new_inventory = old_inventory + new_stock;
        inventory::write(product, new_inventory);
    }
    ...
    fn purchase(product: felt252, quantity: u32) {
        ...
        assert(inventory::read(product) > quantity, 'purchase > inventory');
        let old_inventory = inventory::read(product);
        let new_inventory = old_inventory - quantity;
        inventory::write(product, new_inventory);
    }
    ...
    fn get_stock(product: felt252) -> u32 {
        ...
        let product_stock = inventory::read(product);
        product_stock
    }
}
